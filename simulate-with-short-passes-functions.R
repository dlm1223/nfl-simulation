...
freqs<-df.scrimmage[,list(freq=length(play_type),
                                      percent.short.pass=mean(play_type=="pass"& air_yards<10& !is.na(air_yards)& is.sack==F, na.rm=T), 
                                      percent.long.pass=mean(play_type=="pass"&  air_yards>=10& !is.na(air_yards)& is.sack==F, na.rm=T), 
                                      percent.sack=mean(is.sack| (play_type=="pass"& is.na(air_yards)), na.rm=T), 
...
head(stateDF, 10)
#seems that sacks happen more often in states w. long passes, i.e. sack-rate higher on long passes
summary(lm(percent.sack~scale(percent.short.pass)+scale(percent.long.pass)+as.factor(down), data=stateDF)) 


head(stateDF)
summary(rowSums(stateDF[, grepl("percent", colnames(stateDF))]))
stateDF[rowSums(stateDF[, grepl("percent", colnames(stateDF))])>1, ]
...
###ADJUST STRATEGY FUNCTION####
#function which inputs a set of stateDF rows, increase.percent, and increase.var and outpouts new strategy
# --eventually want to find a full strategy that is optimized for each state 
